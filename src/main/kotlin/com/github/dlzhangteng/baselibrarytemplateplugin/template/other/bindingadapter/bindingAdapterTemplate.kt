...
val bindingAdapterTemplate
    get() = template {
//        revision = 1
        name = "ZTBindingAdapter"
        description = "一键创建 BindingAdapter "
        minApi = MIN_API
        ...
        val mAdapterPackageName = stringParameter {
            ...
            visible = { !isNewModule }
            help = "BindingAdapter 包名"
        }

        val mBeanName = stringParameter {
            name = "BindingAdapter Bean Name"
            constraints = listOf(Constraint.UNIQUE, Constraint.NONEMPTY)
            default = "Bean"
            suggest = { "${mPageName.value}Bean" }
            help = "BindingAdapter 的数据类"
        }

        val mAdapterName = stringParameter {
            name = "BindingAdapter Name"
            constraints = listOf(Constraint.UNIQUE, Constraint.NONEMPTY)
            default = "Adapter"
            suggest = { "${mPageName.value}Adapter" }
            help = "BindingAdapter 的Adapter"
        }
